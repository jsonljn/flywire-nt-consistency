...
if LITERATURE_VALIDATED.exists():
    ...
    check("16 literature-confirmed types", len(confirmed) == 16, f"got {len(confirmed)}")
...
if fafb_corr.exists():
    ...
    check("1297 FAFB neuron corrections", len(fc) == 1297, f"got {len(fc)}")
    check("16 cell types in FAFB corrections", fc["cell_type"].nunique() == 16, f"got {fc['cell_type'].nunique()}")
