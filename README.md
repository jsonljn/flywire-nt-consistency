...
**Known gap, not yet resolved:** R1-6's histamine classification is well-supported by the evidence above, but its 4,090 neurons currently do **not** appear in `corrections/corrections_fafb.csv`, `results/general_scan_n10_full.csv`, `results/three_confusion_patterns.csv`, or `results/literature_validated_candidates.csv` (verified: zero matches for "R1-6" in all four). It is silently dropped inside `general_scan_n10.py`'s per-type loop.

The most likely cause, on current evidence: `resolve_fafb_to_mcns` only reaches R1-6 via range-notation matching (`"R1-6" -> "R1-R6"`), which requires MCNS's `primary_type` column to contain that *exact* combined string. R7 and R8 do **not** rely on this path: both have an explicit entry in `EXPLICIT_SUBTYPE_GROUPS` (`mcns_matching.py`) because MCNS is known to split them into named subtypes (`R7y`, `R7p`, ...). R1-6 has no such entry. If MCNS also splits R1-6's photoreceptor subtypes into individual names rather than one combined `"R1-R6"` string, the same way it does for R7/R8, range notation cannot find it and there is no subtype-group fallback to catch that case. `tests/test_core.py::test_r1_6_has_no_subtype_fallback_unlike_r7_r8` verifies this failure mode directly.

An earlier hypothesis, that the drop happens *after* a successful match, at the `mcns_frac < 0.9` / `mcns_n < 10` threshold check, was investigated first but is not well-supported: it rested on `test_range_notation`, which only proves the matcher works against a synthetic name list that already assumes `"R1-R6"` is the real spelling, not against real MCNS data. `results/full_cross_dataset_scan.csv` records R1-6's match method as `"variant (R1-6 -> R1-R6)"`, a label the current `resolve_fafb_to_mcns` can never produce; that file was generated by matching logic no longer present in this codebase and should not be read as evidence the current matcher succeeds. Do not trust that file's R1-6 row.

Not fixed here: the real `data/data_mcns/merged_annotations.csv` is required to confirm the exact MCNS spelling and is not in this repo (gitignored, downloaded separately from Codex). `general_scan_n10.py` and `full_histamine_scan.py` now log a loud `WARNING` and the exact drop reason if R7, R8, R1-6, or Lai fail to reach `results` on a real run, so this cannot happen silently again. Whoever next runs this pipeline against the real data: check that warning output first.
...
**R1-6 is not in any of these buckets.** It never becomes a candidate for this table: it is dropped upstream in `general_scan_n10.py`'s MCNS cross-check, before `validate_against_literature.py` ever sees it (see the "Known gap" note under "Headline result" above). This is a pipeline gap, not a literature-confirmation failure.
...
Final count: 1,390 FAFB corrections across 18 cell types. This total does **not** include R1-6 (4,090 neurons); see "Known gap" above. 25/25 `validate_results.py` checks pass; `tests/test_core.py` passes in full (see file for current count).
