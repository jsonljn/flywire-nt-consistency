...
Final count: 1,390 FAFB corrections across 18 cell types, as committed. This number predates the R7/R8/R1-6 fix above and has not been regenerated through the full pipeline in this session; see "Headline result" for what has been directly re-verified against real MCNS data (R1-6, R7, R8, Lai all now resolve correctly at the `general_scan_n10.py` stage). 25/25 `validate_results.py` checks pass against the committed files; `tests/test_core.py` passes in full (see file for current count).
