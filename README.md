...
**Code fix verified, not yet propagated into committed results.** Two guesses were made about this gap before real MCNS data was available, and both turned out to be wrong (kept in CHANGELOG rather than deleted). The actual bug, verified against real MCNS v1.0 data: `EXPLICIT_SUBTYPE_GROUPS["R7"]` and `["R8"]` (`mcns_matching.py`) included `"ExR7"`/`"ExR8"`. Those are Extrinsic Ring neurons of the ellipsoid body (Hanesch et al. 1989), a cell class unrelated to R7/R8 photoreceptors that happens to share the name substring. ExR7 (n=4) is 100% ACH while true R7 photoreceptor subtypes are 100% HIST where predicted, so every R7/R8 lookup failed `all_subtypes_consistent` and both were silently dropped. R1-6 was never actually broken; it resolves via range notation exactly as originally written, and reproduces the headline numbers exactly: 4,090 neurons, 81.9% predicted ACH, MCNS confirms 750/750 HIST. Fixed by removing ExR7/ExR8 from `EXPLICIT_SUBTYPE_GROUPS`, and verified live against a real Codex MCNS v1.0 download: no drop warning, R7/R8/R1-6/Lai all resolve correctly.

That fix is *not* reflected in the committed `results/*.csv` files, deliberately. Regenerating `general_scan_n10.py`'s output against real data requires `results/entropy_raw_n10.csv` (the n>=10 sensitivity screen), which is missing from this repo and needs real FAFB data (`data/merged_annotations.csv`) to rebuild. Without it, `general_scan_n10.py` silently falls back to the coarser n>=20 `entropy_raw.csv`, and the resulting match list drops from 381 types to 245: it gains R1-6 but loses 136 legitimate n=10-19 matches (including, e.g., `ORN_DA3` and `Dm19`, both already in the committed correction lists). That trade is a net loss for a deliverable, so the committed results were left as-is rather than overwritten with a smaller, "fixed" file. See CHANGELOG for the full accounting.
...
`general_scan_n10.py` and `full_histamine_scan.py` still log a `WARNING` with the exact drop reason if any of R7/R8/R1-6/Lai fail to reach `results`, so a regression here can't be silent again, once someone has the full data to actually run them end to end.
...
`signature_scan.py` needs only `results/entropy_raw.csv` (or `entropy_raw_n10.csv`). Its literature cross-check (`gt_verified_nt`/`gt_agrees_with_pattern` columns, and `build_signature_corrections.py`) additionally needs `data/gt_data.csv`, the same file as the "Deliverable" section below; download from [flyconnectome/drosophila_neurotransmitters](https://github.com/flyconnectome/drosophila_neurotransmitters).
...
**R1-6 is not literature-confirmed through this table.** It's confirmed through the MCNS + literature evidence under "Headline result" above, not through `validate_against_literature.py`'s literature-database match (no `gt_data.csv` entry exists for it specifically). Regenerating this table against the ExR7/ExR8 fix needs both `data/gt_data.csv` (literature ground truth) and `results/entropy_raw_n10.csv` (FAFB n>=10 screen), neither available in this environment; see "Headline result" and CHANGELOG for what's actually been verified versus what's still blocked.
...
Final count: 1,390 FAFB corrections across 18 cell types, as committed. This number predates the ExR7/ExR8 fix above. It has deliberately not been regenerated: doing so needs both `data/gt_data.csv` and `results/entropy_raw_n10.csv`, neither available here, and a partial regeneration without them was tried and rejected (see CHANGELOG) since it lost far more real matches than it gained. What has been directly verified against real data: R1-6, R7, R8, and Lai all now resolve correctly at the `general_scan_n10.py`/`full_histamine_scan.py` stage. 25/25 `validate_results.py` checks pass against the committed files; `tests/test_core.py` passes in full (see file for current count).
